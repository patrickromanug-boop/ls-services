...
class SupabaseClient(private val app: Application) {
    ...
    companion object {
        ...
        private const val BASE_URL = "https://aihnpdxpzwlhxfmyjmfa.supabase.co"
        private const val ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..." // truncated for brevity
    }

    val isRealConfigActive = true   // toggle for offline mode
    ...
    suspend fun login(email: String, password: String): AuthResult = withContext(Dispatchers.IO) {
        // Simulated: always succeed with demo user
    ...
    fun logout() { /* clear local data */ }

    // ── Jobs ──
    suspend fun fetchJobs(): ApiResult<List<MockJob>> = withContext(Dispatchers.IO) {
        // In real app, call Supabase REST; here return empty to fall back to cache
    ...
    suspend fun insertApplication(userId: String, jobId: String, cvUrl: String, docs: List<String>, title: String, org: String): ApiResult<UserApplication> =
        ApiResult.Success(UserApplication(id = UUID.randomUUID().toString(), userId = userId, jobId = jobId, generatedCvUrl = cvUrl, documentsAttached = docs, status = "pending", appliedAt = java.time.Instant.now().toString(), jobTitle = title, jobOrganization = org))
